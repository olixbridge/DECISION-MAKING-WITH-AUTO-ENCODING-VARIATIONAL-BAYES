...
def model_evaluation_loop(
    ...
    return {
        ...
        "CUBO": my_trainer.test_set.cubo(
            5000,
           encoder_key=my_encoder_eval_name,
           counts=my_counts_eval,
           z_encoder=my_eval_encoder,
        ),
        "MAE": np.abs(is_cdf_nus - exact_cdfs_nus).mean(0), #L1_IS_ERRS IS= importance sampling
        "oliv": oliv,
        #"Posterior variance":seq_var[it],  # Posterior variance
           # d_inv = np.diag(1.0 / seq_var_item)  # Variationnal posterior precision
        "PSIS": statistics.mean(khat_vals), #KHAT
        "a2_norm": a_2_it,
