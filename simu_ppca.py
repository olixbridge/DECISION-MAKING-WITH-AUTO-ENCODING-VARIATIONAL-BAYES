...
nus = np.geomspace(1e-2, 1e1, num=40) #olivia 40
n_hidden_ranges = [128]
N_HIDDEN = 128
...
for dic in scenarios:
    for t in tqdm(range(n_simu)):
        ...
        trainer = GaussianDefensiveTrainer(
            model, DATASET, train_size=0.8, use_cuda=False, frequency=5
        )
        ...
        for eval_dic in EVAL_ENCODERS:
            ...
            print(res_eval_loop)
            res = {
                #"a": eval_dic.get("encoder_type", None),
